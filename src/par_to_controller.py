"""
This file contains multiple methods aiming to parse SCONE .par files, to modify
...
def apply_par_file_to_series_with_new_range(par_file, series, min_max_ratio, std_ratio):
    '''
    This method extracts parameter values from a par file and applies them on 
    a controller PANDAS Series, with new min, max and std values.
    ...
    par_df = create_new_parameter_distribution(par_df, min_max_ratio, std_ratio)
...
def import_par_df(par_file):
    '''
    This method parses a SCONE parameter files to a PANDAS DataFrame
...
def create_new_parameter_distribution(par_df, min_max_range, std_ratio):
    '''
    This method takes a par DataFrame and creates extra columns with new std 
    and min/max ranges.
...
def states_code_to_string(state_code):
    '''
    This method takes a string with state code and returns the corresponding states string.

    Parameters
    ----------
    state_code : string
        Code for gait state where each element is in ['1'..'5'].

    Returns
    -------
    TYPE
        A string containing the names of the states based off state_code.
...
def apply_par_key_to_series(series, key_name, key_value, states, target, source = None):
    '''
    This method sets a new key value on controller PANDAS Series, based on different filters. 
...
def generate_key_df(par_df):
    '''
    This method takes as input a raw DataFrame from a par file and pre-processes
    its column values and only extracts rows that are not threshold-related.

    Parameters
    ----------
    par_df : PANDAS DataFrame
        raw DataFrame from a par file.

    Returns
    -------
    key_df : PANDAS DataFrame
        Pre-processed, filtered DataFrame.
...
def generate_threshold_df(par_df):
    '''
    This method extracts from a raw par file DataFrame the rows related to thresholds.
    ...
    threshold_df : PANDAS DataFrame
        DataFrame, containing only threshold-related rows.
